...
export default {
  ...
  methods: {
    async find_with_sub_category(subcategory_id){
      let response_products = await axios.get("http://localhost:3000/subcategory/find_by_subcategory_id/"+subcategory_id)
      this.products = []
  ...
  async mounted() {
    let response_products = await axios.get(
      "http://localhost:3000/product/get_products"
    );
    ...
    let reponse_categories = await axios.get(
      "http://localhost:3000/category/get_categories"
    );
